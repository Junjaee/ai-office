...
import { HIDDEN_DEPARTMENTS } from "../../company.config";
    ...
    <div className="world-frame">
      ...
      >
        <div className="world-stage" ref={stageRef} style={{ width: WORLD_W, height: WORLD_H }}>
          ...
          {ROOMS.map((room) => {
            ...
            if (HIDDEN.has(room.id)) {
              return (
                <div
                  key={room.id}
                  className="rm rm-dept rm-hidden"
          ...
          <PropLayer />
          <AgentLayer agents={engine.agents.filter((agent) => !HIDDEN.has(agent.deptId))} register={register} onPick={onPick} />
        </div>
