...
import { COMPANY, HIDDEN_DEPARTMENTS, STORAGE_LINK, WORKSPACE } from "../../company.config";
import { WORKSPACE_LIST } from "../workspaces";

/** 숨긴 부서를 뺀 화면용 목록 */
const HIDDEN = new Set(HIDDEN_DEPARTMENTS);
const VISIBLE_ROOMS = DEPT_ROOMS.filter((room) => !HIDDEN.has(room.id));
const VISIBLE_STAFF = STAFF.filter((seed) => !HIDDEN.has(seed.deptId));
import { useRealStatus, type RealState } from "../status";
...
export default function Home() {
  ...
  // 자동화가 커밋한 실제 상태로 부서 상태를 덮어쓴 스냅샷 (연출 + 실제)
  const real = useRealStatus();
  const merged = useMemo<Snapshot>(() => {
    const deptStatus = { ...snap.deptStatus, ...real.status };
    const values = Object.entries(deptStatus).filter(([id]) => !HIDDEN.has(id)).map(([, v]) => v);
    return {
      ...snap,
      deptStatus,
      stats: {
  ...
  const start = () => {
    ...
    setBriefing(false);
    setView("live");
    showToast(`07:00 — AI 직원 ${VISIBLE_STAFF.length}명이 출근합니다 ✨`);
  };
  ...
  const teams = useMemo(
    () =>
      VISIBLE_ROOMS.map((room) => {
        ...
        const status = merged.deptStatus[room.id] ?? "대기";
        return {
          id: room.id,
          icon: room.icon,
          name: room.name,
  ...
  const onDuty = engine.agents.filter((a) => a.status !== "출근 전" && !HIDDEN.has(a.deptId)).length;
  ...
  return (
    <main className="page-shell">
      <div className="wrap">
        <nav className="app-nav" aria-label="AI Company 화면 전환">
          <div className="brand-chip">
            {COMPANY.logoImage ? (
              <img className="brand-logo" src={COMPANY.logoImage} alt="" />
            ) : (
          ...
          <div className="ws-switch" aria-label="사무실 선택">
            {WORKSPACE_LIST.map((w) => (
              <a key={w.id} href={`/${w.id}`} className={w.id === WORKSPACE.id ? "on" : ""}>
                {w.icon} {w.label}
              </a>
            ))}
          </div>
        ...
        <footer>의원실 AI 오피스 · 자동화 현황</footer>
      </div>
      ...
      {briefing ? <BriefingModal snap={merged} onClose={() => setBriefing(false)} /> : null}
    ...
    <>
      <header className="win hero">
        ...
        <div className="hero-body">
          <div className="hero-copy">
            ...
            <p>AI는 비서, 결정은 대표님. {VISIBLE_ROOMS.length}개 팀 {VISIBLE_STAFF.length}명이 맡은 일을 한 흐름으로 관리해요.</p>
          </div>
      ...
      <section className="summary-grid" aria-label="오늘 업무 요약">
        <article className="metric yellow">
          <span>AI 직원</span>
          <strong>{VISIBLE_STAFF.length}</strong>
          <small>STAFF</small>
        </article>
            ...
            <div className="win-body">
              <div className="section-heading">
                ...
                <div className="filter-tabs" role="group" aria-label="팀 상태 필터">
                  {(["전체", "진행 중", "완료", "오류", "승인 대기", "연동 대기"] as const).map((item) => (
            ...
            <div className="win-body">
              <div className="section-heading">
                <div>
                  ...
                  {real.loadedAt ? (
                    <small>상태 확인 {real.loadedAt.toLocaleTimeString("ko-KR")}</small>
                  ) : (
                    <small>상태 파일 불러오는 중…</small>
                  )}
                </div>
              </div>
                ...
                {Object.values(real.byDept).map((s) => (
                  <li key={s.id}>
                    <b>
                      ...
                      {s.name}
                      <span className={`status-pill ${statusClass[real.status[s.dept] ?? "대기"]}`}>
                        {real.status[s.dept] ?? "대기"}
                      </span>
                    </b>
                    <small>
                      {s.summary} · 마지막 실행 {new Date(s.updated_at).toLocaleString("ko-KR")}
                      {s.next_run ? ` · 다음 ${s.next_run}` : ""}
                    </small>
                    {s.counts ? (
                      <small>
                        누적 {s.counts.total ?? "-"}건 · 신규 {s.counts.new ?? 0} · 교체 {s.counts.replaced ?? 0} · 실패{" "}
                        {s.counts.failed ?? 0}
                      </small>
                    ) : null}
                    {s.log?.length ? (
                      <ul>
                        {s.log.slice(0, 5).map((line, i) => (
                          <li key={i}>{line}</li>
                        ))}
                      </ul>
                    ) : null}
      ...
      <p className="dash-note">
        대표 {CEO.name}({CEO.callsign}) · AI 직원 {teams.length}개 부서 {VISIBLE_STAFF.length}명 · 이 화면은 라이브 오피스와 같은 상태를
        공유해요.
      </p>
    </>
  );
