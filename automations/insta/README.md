...
## 여러 계정 (사용자 결정 2026-09-14)

계정 = `config.actions.yaml` 의 `accounts` 항목 하나(프로필·핸들·부서·시간대). 현재 `aitips`(AI 꿀팁) · `parent`(육아 꿀팁·혜택, 인스타 게시글 2팀) · `benefit`(돈 되는 혜택, 인스타 게시글 3팀). 예약 실행(cron)은 **모든 계정을 한 잡에서 차례로** 돌고, 화면에서 누른 실행은 그 계정만 돈다(`inputs.account`). 계정별 인스타 토큰은 워크플로 `env` 에 `INSTA_<계정>_TOKEN/USER_ID/USERNAME` 세 줄씩 — 비어 있으면 그 계정은 **후보만 뽑고 게시는 건너뛴다**(계정 개설 전). 주제별 편집 기준은 프로필 `pick_rules`, 넓은 피드 필터는 `signal_required`·`exclude_words`. Worker 는 실행 제목 접두(`insta_<계정> ·`)로 계정별 최신 실행을 매칭한다(`runBelongsTo`). 새 계정 개설·토큰 절차는 개인 폴더 `ai-side/0N_인스타게시글_<계정>/README.md`.
...
## 카드 → 나레이션 릴스 (기본 게시 형태, 2026-09-14)

팔로워가 없는 계정은 카드뉴스가 아무에게도 배포되지 않는다(팔로워에게만 간다). 릴스는 비팔로워에게도 배포되므로, 영상 소재가 없어도 **렌더한 카드를 세로 영상으로 이어 붙이고 카드 글을 TTS 로 읽는 나레이션 릴스**로 게시한다(`insta_slideshow.py`, `config.actions.yaml` 의 `post_kind: reel`). 무음 릴스는 넘겨지므로(사용자 지적) 나레이션은 필수 — 한국 정보 계정들의 표준(TTS + 자막 + 카드 넘김)과 같다.

- TTS: `edge-tts`(마이크로소프트 엣지 음성, 무료·키 없음). 목소리는 프로필 `tts_voice`(기본 여성 `ko-KR-SunHiNeural`, 남성 `ko-KR-InJoonNeural`). 실패하면 카드뉴스로 되돌린다.
- 카드마다 흐린 확대 배경 + 카드 본체(느린 확대), 길이 = 나레이션 + 0.6초, 첫 카드 최소 3초, 전체 90초 상한(넘으면 뒤 카드부터 뺌).
- 게시 기록 `kind: slideshow`. 대표 화면은 첫 카드.
