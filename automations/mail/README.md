...
2. 찾은 메일을 받은 시각 순으로 모아 **텔레그램 한 통**으로 보낸다(제목·보낸사람·첨부 개수).
   - **지메일 링크는 넣지 않는다**(사용자 결정 2026-09-17).
   - 제목 앞이 라벨과 겹치면 뗀다 — `[재경위] [재경위 행정실] …` → `[재경위] …` (`strip_label_prefix`).
   - 이어서 **첨부를 파일로 보낸다**(`send_attachments: true`). 텔레그램 봇 업로드 한도는 50MB 라
     `max_attachment_mb`(기본 45)를 넘는 파일은 텔레그램으로만 건너뛰고 "보내지 못했어요" 한 줄을 덧붙인다.
3. **첨부를 드라이브에 저장한다** — 위원회 규칙의 `folder_id` 폴더(재경위 = 직장/국회/01. 의정활동/02. 위원회_회의/재경위/_수신함).
   같은 이름이 이미 있으면 올리지 않는다(같은 자료가 여러 메일에 붙어 온다). `folder_id` 가 없는 위원회는 텔레그램으로만 보낸다.
   드라이브 저장은 크기 제한이 없어 큰 자료집도 들어간다.
4. 보내기가 성공하면 메일마다 라벨을 붙이고 `INBOX` 를 뗀다. 라벨이 없으면 만든다.
