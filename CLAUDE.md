...
> **이 폴더 = 저장소 (사용자 결정 2026-09-10).** 이 폴더(`G:\내 드라이브\dev\자동화`) 자체가 git 저장소(GitHub `Junjaee/ai-office`, **공개**)이자 **구글 드라이브 동기화 폴더**다.
> **저장소가 공개라서**(2026-09-16 확인, 사용자 결정: 그대로 둔다) 커밋되는 것은 누구나 본다 — 코드·워크플로·상태 파일(`public/status/`)·실행 일지(`history/`) 전부. 그래서 **메일 제목·보낸사람·일정 내용처럼 의원실 내부 정보는 상태 파일·일지에 넣지 않고 건수만 남긴다**(`automations/mail/mail_gmail.py` 의 `safe_lines()` 가 본보기). 자세한 내용은 텔레그램 등 받는 사람에게만 보낸다. 개인 폴더(`ai-*/`)와 비밀값은 `.gitignore` 로 막혀 있다. 코드·워크플로·대시보드·문서·템플릿이 전부 여기 있고, 커밋·푸시하면 GitHub Actions·대시보드에 반영된다. 다른 곳에 복사본을 두고 작업하지 않는다.
...
- **시각이 아니라 '일이 생겼을 때' 도는 자동화**도 있다: 상임위 메일(`mail.yml`)은 예약표에 없고, Worker 가 1분마다 지메일을 확인해 정리 안 된 국회 메일이 있을 때만 깨운다(`worker/gmail.ts`, 사용자 결정 2026-09-17). 빈 실행이 하루 1,440건 쌓이는 것을 막으려는 것이다. 확인에 필요한 `GOOGLE_*` 는 **Cloudflare Worker 비밀값**에도 넣어야 한다(GitHub Secrets 와 별개).
