...
class ForwardWorker(ctx: Context, params: WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        if (account == null) { log.add(sender, body, false, "설정 파일 없음 — [설정 파일 불러오기]"); return Result.failure() }

        val client = GmailClient(account)
        ...
        return when (Failure.classify(status)) {
            ...
            Outcome.FAIL -> { log.add(sender, body, false, if (token == null) "토큰 오류 HTTP $status — 설정 파일 다시 발급" else "거부 HTTP $status"); Result.failure() }
        }
    }
